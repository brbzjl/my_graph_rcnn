...
from lib.roi_data_layer.roidb import combined_roidb  # make.sh
...
def parse_args():
    ...
    parser.add_argument('--r', dest='resume',
                        help='resume checkpoint or not',  ## False: start from tabula rosa
...
if __name__ == '__main__':
    ...
    if args.dataset == "vg1":
        imdb_name = ["vg_150-50-20_" + s for s in args.imdb_name]  ## redudant
        imdb_name = "vg_150-50-20_" + args.imdb_name
        args.set_cfgs = ['ANCHOR_SCALES', '[4, 8, 16, 32]', 'ANCHOR_RATIOS',
                         '[0.25, 0.5, 1, 2, 4]']  ## followings are the same
    ...
    # train set
    # -- Note: Use validation set and disable img_paththe flipped to enable faster loading.
    cfg.TRAIN.USE_FLIPPED = True
    ### todo: ?? forget what to do
    ...
    cfg.TRAIN.OBJECT_CLASSES = imdb._classes                ## <list> of str
    ...
    if args.save_dir is "local":
        ...
        output_dir = "/home/yijinhui/Projects/VRD/master_thesis/data/models/" + args.net
    ...
    dataset = roibatchLoader(roidb, ratio_list, ratio_index, args.batch_size, \
                             imdb.num_classes, training=True, normalize=False)  ## <roibatchLoader>
    ...
    if not args.resume:
    ...
    else:   ## todo: ?? not read yet
        load_name = os.path.join(output_dir,
                                 'graph_rcnn_{}_a{}_r{}_{}_{}_{}.pth'.format(args.dataset,
        ...
        # load_name = '/srv/share/jyang375/models/vgg16/test.pth'
    ...
    max_step = int(train_size /args.batch_size)
    for epoch in range(args.start_epoch, args.max_epochs):
        ...
        for step in range(max_step):
            data = data_iter.next()     # data(?,3,901,600), im_info(?,3), gt_boxes(?,50,71), num_boxes(?)
            im_data.data.resize_(data[0].size()).copy_(data[0])
            im_info.data.resize_(data[1].size()).copy_(data[1])     ## w, h, scale
            ...
            if (step+1) % accumulation_steps == 0:  ### Gradient Accumulation
                optimizer.step()
                graphRCNN.zero_grad()   ###
                optimizer.zero_grad()   ###
            ...
            if step % args.disp_interval == 0:
                ...
                if args.use_mGPUs > 0:
                    print("[session %d][epoch %2d][iter %4d / %4d] loss: %.4f, lr: %.2e, elapsed time: %d" \
                          % (args.session, epoch, max_step, step, loss_temp, lr, time.time() - origin_time))    ### max_step, origin_time
                    ...
                    print(
                        "\t\t\trpn_cls: %.4f, rpn_box: %.4f, rcnn_obj_cls: %.4f, rcnn_box: %.4f, rcnn_att_cls: %.4f, relpn_clss: %.4f, rcnn_rel_cls: %.4f" %
        ...
        #   pdb.set_trace()
        save_name = os.path.join(output_dir, 'graph_rcnn_{}_a{}_r{}_{}_{}_{}.pth'.format(args.dataset,
                                                                                         1 if cfg.HAS_ATTRIBUTES else 0,
                                                                                         1 if cfg.HAS_RELATIONS else 0,
        ...
        save_checkpoint({
            ...
            'epoch': epoch + 1,                                     ##
            # 'model': graphRCNN.module.state_dict(),                 ### if args.use_mGPUs > 0:
            'model': graphRCNN.state_dict(),                      ### if args.use_mGPUs == 0
