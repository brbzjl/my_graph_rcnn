...
class vg(imdb):
    def __init__(self, version, image_set, ):
        ...
        self._img_path = os.path.join(cfg.DATA_DIR, 'vg')   ## '/home/yijinhui/Projects/VRD/graph-rcnn-bkup-master/data/genome'
        ...
        with open(os.path.join(self._data_path, self._version, 'objects_vocab.txt')) as f:
          ...
          for object in f.readlines():
            names = [n.lower().strip() for n in object.split(',')]  ## todo:??
            self._classes.append(names[0])
            for n in names:
              self._class_to_ind[n] = count ## ind from 1 ~ ...
            count += 1

        # Load attributes
        self._attributes = ['__no_attribute__']
        self._attribute_to_ind = {}
        self._attribute_to_ind[self._attributes[0]] = 0     ## {'__no_attribute__':0, 'attri1':1, ...}
        ...
        self._relation_to_ind[self._relations[0]] = 0       ## {'__no_relation__':0, 'rela1':1, ...}
        ...
        if os.path.exists(os.path.join(self._data_path, "vg_image_index_{}.p".format(self._image_set))):
            with open(os.path.join(self._data_path, "vg_image_index_{}.p".format(self._image_set)), 'rb') as fp:
                self._image_index = pickle.load(fp)     ## [2343962, 498225, ...] todo:?? what order?
        ...
        if os.path.exists(os.path.join(self._data_path, "vg_id_to_dir_{}.p".format(self._image_set))):
            with open(os.path.join(self._data_path, "vg_id_to_dir_{}.p".format(self._image_set)), 'rb') as fp:
                self._id_to_dir = pickle.load(fp)   ## e.g. {id1:'VG_100K_2', id2:'VG_100K', id3...}
    ...
    def image_name_at(self, i):
        ...
        Return the absolute path to image i in the image sequence.
        """
        return self.image_name_from_index(self._image_index[i])
    ...
    def image_name_from_index(self, index):
        ...
        file_name = str(index) + self._image_ext
    ...
    def _load_image_set_index(self):
        ...
        image_index = []            ## todo: ?? followings not readed yet
        id_to_dir = {}
    ...
    def gt_roidb(self):
        ...
        if os.path.exists(cache_file):
            fid = gzip.open(cache_file,'rb')        ## todo:?
        ...
        gt_roidb = [self._load_vg_annotation(index)
                    for index in self.image_index]      ## self.image_index: ## e.g. [2343962, 498225, ...]
        fid = gzip.open(cache_file,'wb')
        cPickle.dump(gt_roidb, fid, cPickle.HIGHEST_PROTOCOL)
        fid.close()
        print 'wrote gt roidb to {}, elapsed time: {}'.format(cache_file, time.time()-start_time)   ###
        return gt_roidb
    ...
    def _load_vg_annotation(self, index):
        ...
        format.
        """
        width, height = self._get_size(index)       ## <int>## todo: time consuming
        filename = self._annotation_path(index)     ## e.g. .../xml/1.xml
        tree = ET.parse(filename)                   ## todo:???         ## time consuming
        objs = tree.findall('object')               ## todo??
        num_objs = len(objs)
        ...
        # "Seg" area for pascal is just the box area        ## todo:??
        ...
        ix = 0
        for obj in objs:
            obj_name = obj.find('name').text.lower().strip()        ## todo:??
            if obj_name in self._class_to_ind:
                bbox = obj.find('bndbox')                   ## todo:??
                x1 = max(0,float(bbox.find('xmin').text))               ## <float>
                y1 = max(0,float(bbox.find('ymin').text))               ## <int>
                x2 = min(width-1,float(bbox.find('xmax').text))         ## <int>
                y2 = min(height-1,float(bbox.find('ymax').text))        ## <float>
                ...
                for att in atts:
                    ...
                    if att in self._attribute_to_ind:
                        ...
                        n += 1
                    if n >= 16:         ## Max of 16 attributes are observed in the data
                        break
                boxes[ix, :] = [x1, y1, x2, y2]         ## <uint16>
                gt_classes[ix] = cls    ## class index, not real class
                overlaps[ix, cls] = 1.0     ## <int 32>
        ...
        return {'boxes' : boxes,                    ## roidb
                ...
                'height': height,
                'flipped' : False,
                'seg_areas' : seg_areas}            ## square area
