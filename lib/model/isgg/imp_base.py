...
class Message_Passing_Unit_v1(nn.Module):
	...
	def forward(self, unary_term, pair_term):
		...
		gate = F.sigmoid(self.w(gate)).sum(1)
		# print 'gate', gate
...
class _IMP_BASE(nn.Module):
	def __init__(self, fea_size, dropout=False, gate_width=1, use_kernel_function=False):
		...
		self.vert_rnn = nn.GRUCell(512, 512)
		self.edge_rnn = nn.GRUCell(512, 512)

                self.gate_vert2edge = Message_Passing_Unit(fea_size, gate_width) 
		self.gate_edge2vert = Message_Passing_Unit(fea_size, gate_width)
