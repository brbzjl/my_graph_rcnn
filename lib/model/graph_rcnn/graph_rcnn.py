...
from model.graph_conv.graph_conv_general import _GraphConvolutionLayer as _GCN_5
...
class _graphRCNN(nn.Module):
    ...
    def __init__(self, baseModels, obj_classes, att_classes, rel_classes, dout_base_model, pooled_feat_dim):
        ...
        if cfg.GCN_LAYERS > 0:
            ...
            if cfg.GCN_ON_FEATS and cfg.GCN_DIFF_FEAT_DIM:
                self.GRCNN_gcn_feat = _GCN_5(cfg.OBJECT_DIM, cfg.ATTRIBUTE_DIM, cfg.RELATION_DIM)
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, use_gt_boxes = False):
        ...
        ## Proposal Layer
        if not use_gt_boxes:
            # feed base feature map tp RPN to obtain rois, 1 x 300 x 5
            ...
            rois = gt_boxes.data.new(gt_boxes.size(0), gt_boxes.size(1), 5)
            rois[0, :, 0] = 0
            rois[0, :, 1:] = gt_boxes.data[0, :, :4]

            if rois.size(1) == 1:
                rois = rois.expand(rois.size(0), 2, rois.size(2))
            rpn_loss_cls = 0
            rpn_loss_bbox = 0
        ...
        obj_cls_score = self.RCNN_obj_cls_score(x_obj)  ## (bs*rois.shape[1], num_class_obj)
        obj_cls_prob = F.softmax(obj_cls_score)
        bbox_pred = self.RCNN_bbox_pred(x_obj)          ## (bs*rois.shape[1]=256, 4)
        ...
        if cfg.HAS_RELATIONS:
            ...
            rel_feats = obj_cls_score.view(rois.size(0), rois.size(1), obj_cls_score.size(1))  ## (bs, ??=128, num_class_obj)
            ...
            x_pred = self._head_to_tail_rel(pooled_pred_feat)                   # (256, 4096)

            if cfg.DETACH_RELATIONS:
                x_rel = x_pred.detach() ## todo:??
            else:
        ...
        if cfg.GCN_ON_FEATS and cfg.GCN_LAYERS > 0:
            ...
            for i in range(cfg.GCN_LAYERS):
                ...
                x_obj, x_att, x_pred = self.GRCNN_gcn_feat(x_obj, x_att, x_pred, map_obj_att, map_obj_obj, map_obj_rel)
        ...
        if cfg.GCN_ON_SCORES and cfg.GCN_LAYERS > 0:
            # compute the intiial maps, including map_obj_att, map_obj_obj and map_obj_rel
            # NOTE we have two ways to compute map among objects, one way is based on the overlaps among object rois.
            # NOTE the intution behind this is that rois with overlaps should share some common features, we need to
            # NOTE exclude one roi feature from another.
            ...
            if cfg.GCN_HAS_ATTENTION:
                x_sobj = obj_cls_score[ind_subject]
                x_oobj = obj_cls_score[ind_object]
                attend_score = self.GRCNN_gcn_att2(x_sobj, x_oobj) # N_rel x 1
                attend_score = attend_score.view(1, x_pred.size(0))
            else:
                attend_score = Variable(x_rel.data.new(1, x_pred.size(0)).fill_(1))
            ...
            # if cfg.MUTE_ATTRIBUTES:
            #     att_cls_score = att_cls_score.detach()
            #     map_obj_att.zero_()
            ...
            # if cfg.MUTE_RELATIONS:
            #     rel_cls_score = rel_cls_score.detach()
            #     map_obj_rel.zero_()
            ...
            for i in range(cfg.GCN_LAYERS):
                ...
                obj_cls_score, att_cls_score, rel_cls_score =\
                    self.GRCNN_gcn_score(obj_cls_score, att_cls_score, rel_cls_score, map_obj_obj, map_obj_att, map_obj_rel)
        ...
        if cfg.HAS_ATTRIBUTES and cfg.HAS_RELATIONS:
            if self.training:
                ...
                rel_pairs = roi_pair_proposals
                # rel_cls_prob = rel_cls_prob * Variable(roi_rel_pairs_score)
