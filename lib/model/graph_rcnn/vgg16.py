...
class vgg16(_graphRCNN):
  def __init__(self, obj_classes, att_classes, rel_classes, d_top_feat, resume=False):
    ...
    base_net = nn.Sequential(*list(vgg.features._modules.values())[:-1])    ## todo: ???

    self.dout_base_model = 512    ## todo:???
    self.d_top_feat = d_top_feat
    _graphRCNN.__init__(self, base_net, obj_classes, att_classes, rel_classes, self.dout_base_model, self.d_top_feat)

    self.RCNN_top = nn.Sequential(*list(vgg.classifier._modules.values())[:-1]) ## todo:???
    if self.d_top_feat != 4096:
      self.RCNN_top_fc = nn.Linear(4096, self.d_top_feat) ## todo:???
    ...
    self.RCNN_bbox_pred = nn.Linear(self.d_top_feat, 4)     ## 4: bounding box corners
