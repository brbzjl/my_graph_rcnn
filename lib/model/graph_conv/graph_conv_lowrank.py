...
from .graph_conv_lowrank_unit import _GraphConvolutionLayer_Collect
from .graph_conv_lowrank_unit import _GraphConvolutionLayer_Update
import numpy as np
import math
import pdb
import time
...
class _GraphConvolutionLayer(nn.Module):
    ...
    def __init__(self, dim):
        ...
        dim_lr = cfg.GCN_LOW_RANK_DIM
        self.gcn_collect = _GraphConvolutionLayer_Collect(dim, dim_lr)
    ...
    def forward(self, feat_obj, feat_att, feat_rel, map_obj_att, map_obj_obj, map_obj_rel):
        ...
        source = self.gcn_collect(feat_att, feat_obj, map_att_obj, cfg.COLLECT_ATT_FROM_OBJ)
        feat_att_updated = self.gcn_update(feat_att, source, cfg.UPDATE_ATT_FROM_ALL)
        ...
        source_att = self.gcn_collect(feat_obj, feat_att, map_obj_att, cfg.COLLECT_OBJ_FROM_ATT)
        source_obj = self.gcn_collect(feat_obj, feat_obj, map_obj_obj, cfg.COLLECT_OBJ_FROM_OBJ)
        ...
        source_rel_sub = self.gcn_collect(feat_obj, feat_rel, map_sobj_rel, cfg.COLLECT_SOBJ_FROM_REL)
        source_rel_obj = self.gcn_collect(feat_obj, feat_rel, map_oobj_rel, cfg.COLLECT_OOBJ_FROM_REL)
        source2obj_all = (source_att + source_obj + source_rel_sub + source_rel_obj)
        ...
        feat_obj_updated = self.gcn_update(feat_obj, source2obj_all, cfg.UPDATE_OBJ_FROM_ALL)
        ...
        source_obj_sub = self.gcn_collect(feat_rel, feat_obj, map_rel_sobj, cfg.COLLECT_REL_FROM_SOBJ)
        source_obj_obj = self.gcn_collect(feat_rel, feat_obj, map_rel_oobj, cfg.COLLECT_REL_FROM_OOBJ)
        source2rel_all = (source_obj_sub + source_obj_obj) / 2
        feat_rel_updated = self.gcn_update(feat_rel, source2rel_all, cfg.UPDATE_REL_FROM_ALL)
