...
class _GraphConvolutionLayer_Collect(nn.Module):
    ...
    def __init__(self, dim):
        ...
        self.collect_units.append(_Collection_Unit(dim)) # att from obj
        self.collect_units.append(_Collection_Unit(dim)) # obj from att
        self.collect_units.append(_Collection_Unit(dim)) # obj from obj
        self.collect_units.append(_Collection_Unit(dim)) # obj (subject) from rel
        self.collect_units.append(_Collection_Unit(dim)) # obj (object) from rel        
        self.collect_units.append(_Collection_Unit(dim)) # rel from obj (subject)
        self.collect_units.append(_Collection_Unit(dim)) # rel from obj (object)
