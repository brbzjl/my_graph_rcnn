...
class _GraphConvolutionLayer_Collect(nn.Module):
    ...
    def __init__(self, dim, dim_lr):
        ...
        self.collect_units.append(_Collection_Unit(dim, dim_lr)) # att from obj
        self.collect_units.append(_Collection_Unit(dim, dim_lr)) # obj from att
        self.collect_units.append(_Collection_Unit(dim, dim_lr)) # obj from obj
        self.collect_units.append(_Collection_Unit(dim, dim_lr)) # obj (subject) from rel
        self.collect_units.append(_Collection_Unit(dim, dim_lr)) # obj (object) from rel
        self.collect_units.append(_Collection_Unit(dim, dim_lr)) # rel from obj (subject)
        self.collect_units.append(_Collection_Unit(dim, dim_lr)) # rel from obj (object)
...
class _GraphConvolutionLayer_Update(nn.Module):
    ...
    def __init__(self, dim):
        ...
        self.update_units.append(_Update_Unit(dim)) # att from obj
        self.update_units.append(_Update_Unit(dim)) # obj from att
        self.update_units.append(_Update_Unit(dim)) # obj from obj
        # self.update_units += _Update_Unit(dim) # obj (subject) from rel
        # self.update_units += _Update_Unit(dim) # obj (object) from rel
        # self.update_units += _Update_Unit(dim) # rel from obj (subject)
        # self.update_units += _Update_Unit(dim) # rel from obj (object)
